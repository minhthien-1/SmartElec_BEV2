...
export class JobDispatchProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(
      `🚀 Processing Dispatch Job: Session #${sessionId}, Attempt #${attempt}`,
    );
    ...
    if (!session || session.status !== 'BROADCASTING') {
      this.logger.log(
        `⚠️ Session #${sessionId} is no longer BROADCASTING. Skipping.`,
      );
      return;
    }
    ...
    if (technicians.length > 0) {
      this.logger.log(
        `✅ [Dispatch] Tìm thấy ${technicians.length} thợ trong bán kính ${radius}km.`,
      );

      const isUrgent = session.isDangerous;
      const title = isUrgent
        ? '🆘 KHẨN CẤP: ĐƠN NGUY HIỂM!'
        : 'Có đơn sửa chữa mới! 🛠️';
      ...
      for (const tech of technicians) {
        if (tech.fcmToken) {
          ...
          this.logger.warn(
            `⚠️ [Dispatch] Thợ #${tech.id} online nhưng không có FCM Token!`,
          );
        }
      }
    } else {
      this.logger.log(
        `ℹ️ [Dispatch] Không tìm thấy thợ nào online trong bán kính ${radius}km quanh Khách hàng.`,
      );
    }
  ...
  private async findNearbyTechnicians(
    lat: number,
    lng: number,
    radiusKm: number,
  ) {
    this.logger.log(
      `📍 [Dispatch] Đang quét thợ quanh vị trí Khách: Lat=${lat}, Lng=${lng} (Bán kính: ${radiusKm}km)`,
    );
